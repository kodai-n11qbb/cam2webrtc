...
## 8. デザインシステム (Design System & Global CSS)

すべてのクライアントページ（`index.html`, `sender.html`, `viewer.html`）は、共通の外部スタイルシート [global.css](file:///Users/abekoudai/Desktop/cam2webrtc/static/global.css) に定義されたデザインシステムに従って統一されています。

### 8.1 デザイン定義とCSS変数 (Design Tokens)
- **カラーパレット**:
  - 背景色 (`--bg-color`): 深い藍色 (`#0b0f19`)。
  - カード背景 (`--card-bg`): 半透明の暗青色 (`rgba(17, 25, 40, 0.65)`)。
  - カード境界線 (`--card-border`): 微細な白透明 (`rgba(255, 255, 255, 0.08)`)。
  - 主要テキスト (`--text-primary`): 明るいグレー (`#f3f4f6`)。
  - 副テキスト (`--text-secondary`): 中間グレー (`#9ca3af`)。
- **背景アニメーション**:
  全ページの背面に、2つの浮遊する半透明カラーオーブ（グラデーションの球体）を配置し、CSSのキーフレームアニメーション（`@keyframes float`）によってゆっくり動き回る演出を施しています。
- **グラスモルフィズム効果**:
  メインコンテンツおよび各カードのコンテナには、`backdrop-filter: blur(12px)` と極薄の白い枠線（Border）を設定し、すりガラスのような立体感を持たせています。
- **共通UIコンポーネント**:
  - **ボタン**: 通常ボタンとプライマリ/サクセスグラデーションボタン。ホバー時に影と拡大（`translateY(-2px)`）のアニメーションを統一。無効化状態（Disabled）は透明度を下げ、クリック不可に。
  - **テキスト入力フィールド**: フォーカス時にインディゴ色のグロー効果（`box-shadow`）が広がる。
  - **ステータスバナー**: `info`, `success`, `error` のそれぞれに対して、背景色と文字色を統一された半透明カラーで表示。
